...
private func deviationOld(previousRecorded: Date, currentRecorded: Date, currentCustom: Date) -> String {
    let driftSeconds = currentCustom.timeIntervalSince(currentRecorded)
    let intervalBetweenRecords = currentRecorded.timeIntervalSince(previousRecorded)
    
    guard intervalBetweenRecords != 0 else { return "0 s" }
    
    let days = intervalBetweenRecords / 86400.0
    let secondsPerDay = driftSeconds / days
    
    let sign = secondsPerDay >= 0 ? "+" : "-"
    
    return "\(sign)\(String(format: "%.2f", abs(secondsPerDay))) s"
}
private func deviation(currentRecorded: Date, currentCustom: Date) -> String {
    let driftSeconds = currentCustom.timeIntervalSince(currentRecorded)
    let sign = driftSeconds >= 0 ? "+" : "-"
    return "\(sign)\(String(format: "%.2f", abs(driftSeconds))) s"
}
// MARK: - ActiveSheet
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 12) {
                ...
                Button {
                    let now = Date()
                    customHour = Calendar.current.component(.hour, from: now)
                    customMinute = Calendar.current.component(.minute, from: now)
                    customSecond = Calendar.current.component(.second, from: now)
                    editingEntryID = manager.entries.first?.id
                    activeSheet = .customTime
                } label: {
                    Label("Add Custom Time", systemImage: "plus.circle")
                        .frame(maxWidth: .infinity)
                }
                .buttonStyle(.bordered)
                .padding(.horizontal)
